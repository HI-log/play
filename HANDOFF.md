...
```js
{
  ...
  hours: "10:00-18:00",     // 운영시간 — 카드에 🕐 표시
  closedOn: ["mon"],        // 휴무 요일 — sun/mon/tue/wed/thu/fri/sat 약자 배열. 오늘 휴무면 카드 dimmed + 🚫 오늘 휴무 배지
  ageMin: 36,               // 권장 연령 하한 (개월). "2~4세 OK" 필터는 ageMin ≤ 48 인 카드만 통과
  ageMax: 84,               // 권장 연령 상한 (개월, 선택)
}
```
...
| 필드 | 효과 |
|---|---|
| `hours: "10:00-18:00"` | 카드에 `🕐 10:00-18:00` |
| `closedOn: ["mon"]` | 카드에 `🚫 월요일 휴무`. 오늘이 휴무요일이면 카드 dimmed + `🚫 오늘 휴무` 빨간 배지. 옵션 필터 `📅 오늘 영업중만`으로 숨기기 가능 |
| `ageMin: 36`, `ageMax: 84` | 카드에 `👶 36~84개월`. 옵션 필터 `👶 2~4세 OK`는 `ageMin ≤ 48`이거나 필드 자체가 비어있는 항목만 통과 |

기존 162곳은 비어있고, 정보 확인된 곳만 점진적으로 채우면 됨.

## 컬렉션 (📚 탭)

`COLLECTIONS` 배열에 정의된 미리 짜둔 묶음들. 5번째 탭 `📚 컬렉션`에서 노출.
...
```js
{
  ...
  icon: "🌧️",
  name: "비 오는 날 BEST",
  desc: "실내 위주로 종일 놀 수 있는 곳",
  items: ["주렁주렁 (하남·코엑스·수원)", "코엑스 아쿠아리움", ...],
}
```

`items`는 `DATA[i].name`과 정확히 또는 일부 일치해야 매칭됨. 매칭 안 되면 자동 skip + 카운트에서 제외. 만료된 이벤트(`endDate < 오늘`)도 자동 제외.

새 컬렉션 추가: `COLLECTIONS` 배열에 객체 한 개 push.

## 거리순 정렬

리스트 뷰 상단 정렬 select. `거리순` 선택 시:
1. localStorage `user-location` 캐시 확인 (1시간 유효)
2. 캐시 없으면 Geolocation 요청
3. 거부되면 "지도에서 직접 찍기" 모달 안내 (또는 추천 탭에서 도시 선택)
4. 캐시 위치 기준으로 `haversineKm`로 거리 계산 후 오름차순 정렬

카드 location 옆에 `📍 12.3km` 작은 텍스트로 거리 표시.

## 위치 설정 방법 3가지

1. **추천 탭 → 📍 추천 받기** — Geolocation 권한 허용 → 정확한 현재 위치
2. **추천 탭 → 도시 선택 dropdown** — 광역시·도 단위 미리 정의된 좌표
3. **🗺️ 지도에서 찍기 모달** — 추천 탭 intro의 버튼 OR 옵션 행의 버튼 OR 거리순 권한 거부 시. 지도 클릭으로 좌표 직접 지정.

설정된 위치는 모두 `localStorage["user-location"]`에 1시간 캐시. 셋 모두 같은 캐시 공유.

## 반경 필터

옵션 행의 select: `5/10/30/50/100km 이내` 또는 `제한 없음`. 위치 캐시 있을 때만 활성. 모든 뷰(리스트·지도·캘린더·컬렉션)에 자동 적용. `state.maxDistKm`로 관리.
